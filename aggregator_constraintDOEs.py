...
def Add_DOE_Constraints(aggregator,model,t):
    ...
    def L_DOE_compliance(m,n,t):
        return m.P_meter[n,t] + m.L_bid[n,t] <= nodes[n]['import_DOE'][t]
    def import_DOE_rule(m,n,t): #(lower bound, expr, upper bound) construction
        return m.P_meter[n,t] <= nodes[n]['import_DOE'][t]+((1-m.Online[n,t])*(nodes[n]['load_max'][t]+nodes[n]['P_batt_max'][t]))
    def export_DOE_rule(m,n,t):
        return m.P_meter[n,t] >= nodes[n]['export_DOE'][t]+((1-m.Online[n,t])*(nodes[n]['P_solar_min'][t]+nodes[n]['P_batt_min'][t]))
...
def Write_All_Data(aggregator,model,t):
    ...
    def node_settlement(n):
        ...
        fcas_rev = (node['raise_price'][t]*node['R_bid'][t] + node['lower_price'][t]*node['L_bid'][t])*conf.interval_duration
        if node['P_meter'][t] >= 0:
