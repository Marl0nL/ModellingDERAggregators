...
import StrategicIOE as stgy

def Aggregator_Model(aggregator,t,mode='prediction',horizon=conf.horizon_intervals):
    #Indices
...
# Objective
    def objective_rule(model):
        ...
        FCAS_revenue = quicksum(
            #       MW                   h                  convert to $/MWh from $/kW*h
            (model.R_bid[n,t]*conf.interval_duration) * nodes[n]['raise_price'][t] + (model.L_bid[n,t] * conf.interval_duration) * nodes[0]['lower_price'][t]
        ...
        import_DOE_Breach_Penalty = quicksum(1000*conf.interval_duration*model.import_doe_breach[n, t] for n in nodes.keys() for t in T)
        export_DOE_Breach_Penalty = quicksum(1000*conf.interval_duration*model.export_doe_breach[n, t] for n in nodes.keys() for t in T)
    ...
    def P_export_bounds(m,n,t):
        return (0, m.P_export[n,t], 100)

    def P_import_bounds(m,n,t):
        return (0, m.P_import[n,t], 100)
    
    def P_export_sign_rule(m,n,t):
        return m.P_export[n,t] <= 10000*(1-m.sign[n,t])
    
    def P_import_sign_rule(m,n,t):
        return m.P_import[n,t] <= 10000*m.sign[n,t]
    ...
    def Aggregate_L_bids(m,t): #MW
        return m.L_bid_agg[t] == quicksum(m.L_bid[n,t] for n in nodes.keys())/1000
    ...
    def Aggregate_R_bids(m,t):
                    #MW             kW
        return m.R_bid_agg[t] == quicksum(m.R_bid[n,t] for n in nodes.keys())/1000
...
def Add_DOE_Constraints(aggregator,model,t):
    nodes = aggregator['Nodes']
    for index in model.P_solar.index_set():
    ...
    def import_DOE_rule(m,n,t):
        return m.P_meter[n,t] - nodes[n]['import_DOE'][t]-0.001 <= 10000000*m.import_doe_breach[n,t] #without the 0.001 error margin, tiny float errors were causing "breaches"
    def export_DOE_rule(m,n,t):
        return nodes[n]['export_DOE'][t] - m.P_meter[n,t]-0.001 <= 10000000*m.export_doe_breach[n,t]
...
def Write_Prediction_Data(aggregator,model,t,IOEs='inactive'):
    nodes = aggregator['Nodes'].keys()
    
    if IOEs == 'inactive':
        for node in nodes:
            aggregator['Nodes'][node]['upper_IOE'][t] = aggregator['Nodes'][node]['P_batt_max'][t] + aggregator['Nodes'][node]['load_max'][t]
            aggregator['Nodes'][node]['lower_IOE'][t] = aggregator['Nodes'][node]['P_batt_min'][t] + aggregator['Nodes'][node]['P_solar_min'][t]
        ...
        if aggregator['Behaviour'] == 'Non-strategic':
            ...
            for node in nodes:
                ...
                aggregator['Nodes'][node]['upper_IOE'][t] = iioe
                aggregator['Nodes'][node]['lower_IOE'][t] = eioe
...
def Write_All_Data(aggregator,model,t):
    ...
    def node_settlement(n):
        ...
        if node['P_meter'][t] >= 0:
            ...
            tariff_cost = -1*node['P_meter'][t]*conf.interval_duration*node['export_price'][t]
    ...
    for node in nodes:
        ...
        aggregator['Nodes'][node]['Upper_DOE_Breach'][t] = model.import_doe_breach[node,t].value
        aggregator['Nodes'][node]['Lower_DOE_Breach'][t] = model.export_doe_breach[node,t].value
