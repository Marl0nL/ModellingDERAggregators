...
def Aggregator_Optimisation(aggregator,t,mode='prediction',horizon=conf.horizon_intervals,IOEs='inactive'):
    #Indices
    ...
    model.R_bid =       Var(nodes.keys(),T,name = 'raise_price',within=NonNegativeReals)
    model.L_bid =       Var(nodes.keys(),T,name = 'lower_price',within=NonNegativeReals)
...
# Objective
    def objective_rule(model):
        ...
        return tariff_cost - FCAS_revenue
    ...
    def delta_SOC_relationship(m,n,t):
        if t == T[-1]:
            return m.SOC[n,t] == m.SOC[n,(t-1)] + m.P_batt[n,t] * nodes[n]['battery_eta'][t] * conf.interval_duration
        return m.SOC[n,(t+1)] == m.SOC[n,t] + m.P_batt[n,t] * nodes[n]['battery_eta'][t] * conf.interval_duration
    ...
    # #If m.sign is 0, P must be negative
    # def sign_constraint_rule1(m,n,t): #Big M method
    #     return m.P_meter[n,t] <= 10000 * m.sign[n,t]
    # #If m.sign is 1, P_meter must be positive.
    # def sign_constraint_rule2(m,n,t):
    #     return -1*m.P_meter[n,t]*(m.sign[n,t]) <= 0
    ...
    def R_battery_energy(m,n,t):
        return m.R_batt[n,t] - m.P_batt[n,t] <= (m.SOC[n,t] - nodes[n]['SOC_min'][t]) / (nodes[n]['battery_eta'][t] * conf.interval_duration)
    ...
    def NonStrategic_Import_IOE_rule(m,n,t):
        return m.import_IOE[n,t] == m.L_bid[n,t]+m.P_meter[n,t]
    
    def NonStrategic_Export_IOE_rule(m,n,t):
        return m.export_IOE[n,t] == m.L_bid[n,t]+m.P_meter[n,t]
    
    #Instead of a fixed calculation method, the hope is that a DSO subproblem nested in the objective
    #will give rise to strategic setting of the IOE, within these bounds.
    ...
    #doe_related
    if mode == 'action': #Don't enforce DOEs in the prediction stage.
        model.doe_constraint = Constraint(nodes.keys(), T, rule=DOE_rule)
    ...
    #model.battery_availability = Constraint(nodes.keys(),T,rule=available_energy_constraint)
...
#Connectivity
    ...
    log.info('Finished building model. Returning model.')
